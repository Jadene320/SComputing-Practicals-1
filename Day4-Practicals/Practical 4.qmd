...
NA value: The was only one distance value (n=1) for month == 1 for that particular carrier.

The mathematical context in which sd = 0, is when the sum of the differences of obs and mean is = 0. So the distance of flights is the same.
...
delay_check <- flights |> 
  ...
  #mutate(prop_init = num_dep_delay/num_arr_delay) |> 
  summarise(
...
#Identify airlines for the routes. HOW?*********
...
JFK to ATL is the route with the greatest diff
